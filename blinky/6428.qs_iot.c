...
volatile bool g_blastErrLed = false;
...
Timer0IntHandler(void)
{
    ...
    //
    // Oncer per half second, perform the following operations.
    //
    if(!(g_ui32TimerIntCount % APP_TICKS_PER_SEC/2))
    {
        if(!g_bOnline)
        {
			if(g_blastErrLed)
    ...
    while(1)
    {
        ...
        if(g_bOnline)
        {
            ...
            else if(Exosite_StatusCode() == EXO_STATUS_NOAUTH)
            {
                ...
                //
                UARTprintf("\nCIK no longer valid. ");
                UARTprintf(",Attempting to auto Activate\n");
            ...
            else
            {
                ...
                {
                	UARTprintf("Connection may have dropped, offline\n");
                    g_bOnline = false;
                }
            }
        }
        ...
        {
        	//
        	// Try to find out why we are offline
        	//
        	//
        	    // Loop a few times to make sure that DHCP has time to find an IP.
        	    //
            ...
            //
            // Attempt to reconnect with Exosite
            //
